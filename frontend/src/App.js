        ...
        <div className="flex h-full bg-[#ece5dd]" style={{fontFamily:'Segoe UI,Roboto,Arial,sans-serif'}}>
          <div className="w-1/3 min-w-[340px] max-w-[400px] bg-[#f0f2f5] border-r flex flex-col h-full shadow-lg">
            <div className="flex items-center gap-3 px-5 py-4 bg-[#ededed] border-b">
              ...
              <button className="ml-auto bg-red-500 text-white px-3 py-1 rounded" onClick={handleLogout}>
                Logout
              </button>
            </div>
            {showProfile && (
            ...
            <div className="flex-1 overflow-y-auto pb-2">
              ...
              {chats.map(chat => (
                <div
                  key={chat._id}
                  className={`flex items-center gap-3 px-5 py-3 cursor-pointer border-b hover:bg-[#e9edef] ${
                    selectedChat && selectedChat._id === chat._id ? 'bg-[#d1f7c4]' : ''
                  }`}
          ...
          <div className="flex-1 flex flex-col bg-[#ece5dd] h-full min-w-0">
            <div className="flex items-center gap-3 px-6 py-4 bg-[#075e54] text-white border-b shadow sticky top-0 z-10">
              {selectedChat && selectedChat.avatar ? (
                <img src={selectedChat.avatar} alt="avatar" className="w-8 h-8 rounded-full" />
              ) : (
                <span className="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center">👤</span>
              )}
              <div className="flex-1">
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-3 flex flex-col-reverse" style={{scrollBehavior:'smooth'}}>
              {messages.slice().reverse().map(msg => (
                <div key={msg.meta_msg_id} className={`flex ${msg.name === user.name ? 'justify-end' : 'justify-start'} transition-all duration-200`}>
                  <div
                    className={`group relative rounded-2xl px-4 pt-2 pb-4 max-w-[70%] shadow ${
                      msg.name === user.name ? 'bg-[#dcf8c6] border border-[#b2e59f]' : 'bg-white border border-gray-300'
                    } animate-fade-in`}
            ...
            {selectedChat && (
              <>
                ...
                <div className="px-8 py-5 border-t bg-[#f7f7f7] flex items-center gap-3">
                  ...
                  />
                  <button
                    className="bg-[#25d366] text-white px-6 py-3 rounded-full font-bold text-lg shadow"
                    onClick={sendMessage}
                  >
                    Send
                  </button>
                ...
                {filePreview && (
                  <div className="px-8 pb-2 flex items-center gap-2">
                    ...
                    <button className="text-red-500" onClick={() => setFilePreview(null)}>
                      Remove
                    </button>
                  </div>
                )}
